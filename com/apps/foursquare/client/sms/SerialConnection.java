...
/*    private TextArea messageAreaOut;
   ...
   */
   public void openConnection() throws SerialConnectionException {
	...
	// wants to.
	try {
	    sPort = (SerialPort)portId.open("SMSConnector", 300000000);
	    System.out.println("prot opened");
	} catch (PortInUseException e) {
		 System.out.println("prot Exception"); 
	    throw new SerialConnectionException(e.getMessage());
	}
    ...
    */
	...
	switch (e.getEventType()) {
	    ...
	    case SerialPortEvent.DATA_AVAILABLE:
	    	...
	    	while (newData != -1) {
		    	//System.out.println("new data is available "+newData);
	    		try {
		    	    newData = is.read();
			    if (newData == -1) {
				break;
		...
		if(receptionString.indexOf("REC READ")!=-1){
			System.out.println("reception string is "+receptionString.trim());
			test te = new test();
		te.parseString(receptionString);
		}
